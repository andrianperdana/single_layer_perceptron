...
TEST_RATIO = (
    0.2  # proporsi data yang digunakan untuk pengujian (80% pelatihan, 20% pengujian)
)
...
)  # hitung std dari data pelatihan, tambahkan epsilon untuk menghindari pembagian dengan nol
...
def binary_cross_entropy(
    y_true, y_pred, eps=1e-12
):  # eps = 1e-12 untuk menghindari log(0)
    y_pred = np.clip(y_pred, eps, 1 - eps)  # avoid log(0) by clipping predictions
    return -(
        y_true * np.log(y_pred) + (1 - y_true) * np.log(1 - y_pred)
    ).mean()  # rumus matematis binary cross-entropy loss (-(y*log(y_pred) + (1-y)*log(1-y_pred)))


# Single Layer Perceptron with Sigmoid Activation
class SigmoidPerceptron:
    def __init__(
        self, n_features, learning_rate=0.1, seed=None
    ):  # self = instance of the class, n_features = jumlah fitur input, learning_rate = laju pembelajaran, seed = untuk inisialisasi bobot acak
        self.rng = np.random.default_rng(seed)  # random number generator
        self.weights = self.rng.normal(
            loc=0.0, scale=0.01, size=(n_features,)
        )  # inisialisasi bobot dengan distribusi normal (mean=0, std=0.01)
    ...
    def predict_proba(self, X):
        ...
        )  # hitung z = Xw + b, lalu aplikasikan fungsi sigmoid
    ...
    def predict(self, X, threshold=0.5):  # threshold default = 0.5
        return (self.predict_proba(X) >= threshold).astype(
            int
        )  # jika probabilitas >= threshold, kelas = 1, else kelas = 0

    # fungsi untuk melatih model
    def fit(
        self, X, y, epochs=100
    ):  # X = fitur input, y = label target, epochs = jumlah iterasi pelatihan
        m, _ = X.shape  # m = jumlah sampel, n = jumlah fitur
        history = {
            "loss": [],
            "accuracy": [],
        }  # history untuk menyimpan riwayat loss dan akurasi
        ...
        for _ in range(epochs):
            ...
            )  # update bobot (w = w - learning_rate * grad_w)
            self.bias -= (
...
model = SigmoidPerceptron(
    n_features=X_train_std.shape[1], learning_rate=LEARNING_RATE, seed=RANDOM_SEED
)
...
test_proba = model.predict_proba(
    X_test_std
)  # prediksi probabilitas pada data pengujian
test_predictions = (test_proba >= 0.5).astype(
    int
)  # prediksi kelas biner pada data pengujian (threshold=0.5)
test_accuracy = (
    test_predictions == y_test
).mean()  # hitung akurasi pada data pengujian

print(
    f"Final training accuracy: {history['accuracy'][-1]:.4f}"
)  # print akurasi akhir pada data pelatihan
